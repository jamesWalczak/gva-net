...
class ModelNet40Splits:
    def __init__(self, num_points, test_size=1 / 10):

        data, label = load_data_cls("train")
        data2, label2 = load_data_cls("test")

        self.data = np.vstack((data, data2))
        self.label = np.vstack((label, label2))
        self.num_points = num_points

        self.train_x, self.test_x, self.train_l, self.test_l = train_test_split(
            self.data, self.label, test_size=test_size, shuffle=True
        )

    def train(self):
        return ModelNet40(
            self.train_x, self.train_l, num_points=self.num_points, train=True
        )

    def test(self):
        return ModelNet40(
            self.test_x, self.test_l, num_points=self.num_points, train=False
        )


class ModelNet40(Dataset):
    def __init__(self, x, y, num_points, train=False):
...
def train(
    ...
    experiment.add_tag("10-fold repeated sample validation")
    ...
    splits = ModelNet40Splits(num_points=used_pts)
    train_loader = torch.utils.data.DataLoader(
        splits.train(), batch_size=bs, shuffle=True
    )
